...
export default function DeleteResistanceRiskPanel({ item, onConfirm, onCancel, isPending, error }: DeletePanelProps) {
    return (
        <div className="flex flex-col h-full bg-white rounded-md shadow-sm border border-gray-200">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 bg-red-50/50 rounded-t-md">
                <h2 className="text-lg font-bold text-red-600">
                    Delete Resistance Risk
                </h2>
            </div>

            <div className="p-6 space-y-5 flex-1">
                <p className="text-sm text-gray-700 leading-relaxed">
                    Are you sure you want to delete the risk <strong className="text-red-600">{item.name}</strong> for pathogen <strong className="text-zinc-800">{item.pathogen}</strong>? This action cannot be undone.
                </p>
