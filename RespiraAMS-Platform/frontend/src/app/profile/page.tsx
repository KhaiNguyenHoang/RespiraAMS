...
const normalizeProfilePayload = (values: ProfileFormState) => ({
	...values,
...
const mapDoctorProfile = (data: Record<string, unknown>): DoctorProfile => ({
	...
	educations: Array.isArray(data.educations) ? (data.educations as Array<{ title: string; school: string; year: string }>) : [],
...
export default function ProfilePage() {
	...
	const handleAvatarChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		try {
			...
			formData.append("CitizenIdentificationCard", normalizedPayload.citizenIdentificationCard);
			formData.append("DateOfBirth", normalizedPayload.dateOfBirth);
			formData.append("Gender", String(normalizedPayload.gender));
			normalizedPayload.degrees.forEach((degree) => formData.append("Degrees", degree));
			formData.append("Avatar", file);

			const response = await fetch(`${API_BASE}/Profile/update/doctor/${user.id}`, {
				method: "PUT",
				headers: {
			...
			if (!response.ok || !result.success) {
				throw new Error(result.message ?? "Cập nhật ảnh đại diện thất bại.");
			}
		} catch (err) {
	...
	const fullName = [doctor.firstName, doctor.lastName].filter(Boolean).join(" ") || "Chưa cập nhật";
	...
	useEffect(() => {
		const loadProfile = async () => {
			...
			try {
				setError(null);
				const response = await fetch(`${API_BASE}/Profile/doctor/${user.id}`, {
					headers: {
						Authorization: `Bearer ${token}`,
					},
				});
				...
				const profile = mapDoctorProfile(result.data ?? {});
				setDoctor(profile);
				setForm({
	...
	const handleChange = (field: keyof ProfileFormState) => (
		...
		if (field === "gender") {
			setForm((prev) => ({ ...prev, gender: event.target.value === "true" }));
			return;
		}
	...
	const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			formData.append("CitizenIdentificationCard", payload.citizenIdentificationCard);
			formData.append("DateOfBirth", payload.dateOfBirth);
			formData.append("Gender", String(payload.gender));
			payload.degrees.forEach((degree) => formData.append("Degrees", degree));
			...
			<div className="mb-6 flex items-start justify-between gap-4">
				<div>
					...
					<p className="mt-1 text-sm text-gray-500">
						Xem và quản lý thông tin chứng chỉ hành nghề và thông tin cá nhân của bác sĩ.
					</p>
				</div>
								...
								<div className="grid gap-4 text-sm sm:grid-cols-2">
									...
									<div>
										...
										<div className="flex items-center gap-2">
											<div>{showCccd ? (doctor.citizenIdentificationCard || "Chưa cập nhật") : maskValue(doctor.citizenIdentificationCard)}</div>
											<button
												type="button"
												onClick={() => setShowCccd((value) => !value)}
									...
									<div>
										...
										<div className="flex items-center gap-2">
											<div>{showEmail ? (doctor.email || "Chưa cập nhật") : maskValue(doctor.email)}</div>
											<button
												type="button"
							...
							<div className="rounded-md border border-gray-200 bg-gray-50 p-3">
								<div className="grid gap-3 sm:grid-cols-3">
									<div>
										...
										<div className="text-sm text-gray-700">{form.email || "Chưa cập nhật"}</div>
									</div>
									<div>
										<div className="text-xs uppercase text-gray-500">CCCD</div>
										<div className="text-sm text-gray-700">{form.citizenIdentificationCard || "Chưa cập nhật"}</div>
									</div>
									<div>
										<div className="text-xs uppercase text-gray-500">Ngày sinh</div>
										<div className="text-sm text-gray-700">{form.dateOfBirth ? new Date(form.dateOfBirth).toLocaleDateString("vi-VN") : "Chưa cập nhật"}</div>
									</div>
								</div>
							</div>
									...
									<div className="space-y-2 rounded-md border border-gray-200 p-3">
										...
										<div className="flex flex-wrap gap-2">
											{degreeOptions.map((option) => {
												const checked = form.degrees.includes(option.value);
												return (
													<label
														key={option.value}
														...
														<input
															...
															onChange={() => {
																setForm((prev) => ({
																	...prev,
																	degrees: checked
																		? prev.degrees.filter((item) => item !== option.value)
																		: [...prev.degrees, option.value],
																}));
															}}
														/>
