...
namespace Application.Sagas.DeleteDoctorSaga
{
    public class DeleteDoctorSagaHandler
    {
        ...
        private readonly ILogger<DeleteDoctorSagaHandler> _logger;
        private readonly string _defaultBucketName;

        public DeleteDoctorSagaHandler(
        ...
        )
        {
            ...
            _logger = logger;
            _defaultBucketName = configuration["R2:BucketName"] ?? "avatars";
        }
        ...
        public async Task<object> Handle(DeleteMediaCommand command)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("DeleteMediaCommand failed: MediaAsset with ID {MediaId} not found or already deleted.", command.MediaId);
                    return new DeleteMediaFailed(command.Id, "Media asset not found or already deleted.");
                }
                ...
                _logger.LogInformation("Media asset database record soft-deleted successfully for Media ID {MediaId}", command.MediaId);
        ...
        public async Task Handle(CleanDeleteMediaCommand command)
        {
            try
            {
                ...
                {
                    await _storageService.DeleteAsync(command.FileName, command.BucketName ?? _defaultBucketName);
                    _logger.LogInformation("Physically deleted media file {FileName} from bucket {BucketName}", command.FileName, command.BucketName);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to physically delete media file {FileName} from bucket {BucketName}", command.FileName, command.BucketName);
            }
        }
    }
}
